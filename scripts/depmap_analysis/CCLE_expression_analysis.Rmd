...
Description: analysis of the RNA-seq expression data classifying it based on the presence of mutations in the spliceosome vs the lack of mutations in the spliceosome. 
...
Load gene set.
I downloaded the Human Metabolic Reaction (HMR) from Sysbio: http://atlas.sysbio.chalmers.se/downloads/hmr

Start with KEGG, reactome and hallmark.
```{r}

```

Prepare input data for piano
```{r}
#WE USE ADJUSTED P-VALUES FROM THE DGESeq ANALYSIS AND LOG 2 FOLD CHANGES
DOUBLE CHECK IF FDR OR RAW P-VALUES
padj = tbl_YvsN %>% 
  pull(FDR)
...
names(log2fc) = tbl_YvsN %>% 
  pull(SYMBOL)
```

Run PIANO
```{r}
gsaRes_GastvsMin <- runGSA(padj,
                 log2fc,
                 gsc=HMR2_gsc,
                 ncpus=8,
