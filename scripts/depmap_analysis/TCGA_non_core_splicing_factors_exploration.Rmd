---
title: "Spliceosome analysis of non-core factors - splicing factors list from TCGA"
output: html_notebook
---

Description: analysis of non-core splicing factors (using list from TCGA). Generated binary heatmaps based on presence / absence of the mutation in a certain cell line.
...
df_ccle_factors = 
  ...
  distinct() %>% 
  select(DepMap_ID, Hugo_Symbol, cDNA_Change, primary_disease, Variant_Type, Annotation_Transcript) 
...
df_factors_category = 
  df_ccle_factors %>% 
  inner_join(df_factors, by = "Hugo_Symbol") %>% 
  distinct(DepMap_ID, Hugo_Symbol, cDNA_Change, .keep_all = T) %>% 
  mutate(Category = ifelse(is.na(Category), "non-core", Category))

head(df_factors_category)
```

Let's filter based on core/non-core
```{r}
nocore_ccle = 
  df_factors_category %>% 
...
```{r, fig.height=3.5, fig.width=4}
  ...
  geom_text(aes(label = sprintf("%.f", round(NORMALIZED_MUTATIONS, digits =2))), size = 2, position = "dodge",    hjust=-0.3) +
  xlab(NULL) + 
...
df_hm_tidy %>%
  heatmap(Gene, DISEASE, Relative_mutation,
