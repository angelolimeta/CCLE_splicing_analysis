...
long_transcripts_pc = 
  long_transcripts_info %>% 
  filter(primary_disease == "Pancreatic Cancer")
...
long_transcripts_pc$ensembl_id <- gsub("\\(|\\)","",as.character(long_transcripts_pc$ensembl_id))
...
lm_pc = 
  lm_res %>% 
  filter(term == "primary_diseasePancreatic Cancer")
...
```{r}
saveRDS(all_p.val, file = "res_lancaster/p_values_lancaster_pc.rds")
```
