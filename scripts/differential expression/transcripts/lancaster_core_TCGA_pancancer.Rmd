---
title: "Lancaster method for linear model results - for mutations in core splicing factors"
...
```{r}
lm_res = readRDS("../github/results/differential_expression/transcripts/tcga_core_spliceosome_lm_res_symbols.rds")
...
mean_spliceosome = 
  df_deseq %>% 
  group_by(spliceosome_mutated, Gene) %>% 
  summarise_at(vars(log_TPM), funs(mean(., na.rm = TRUE)))

mean_disease = 
  df_deseq %>% 
  group_by(primary_disease, Gene) %>% 
  summarise_at(vars(log_TPM), funs(mean(., na.rm = TRUE)))
```
...
lanc = 
  lm_spl %>% 
  left_join(mean_spliceosome_yes, by = c("Hugo_Symbol", "ensembl_id")) %>% 
...
```{r}
saveRDS(all_p.val, file = "res_lancaster/core_tcga_p_values_lancaster.rds")
```
