---
title: "Lancaster method for linear model results - for mutations in other splicing factors"
...
mutations_spliceosome = fread("/Users/castilln/Desktop/thesis/localdata/spliceosome/tcga_other_spliceosome_ccle_mutations.csv")
```

Load lm results from transcripts_lm
```{r}
lm_res = readRDS("../github/results/differential_expression/transcripts/tcga_other_spliceosome_lm_res_symbols.rds")
...
long_transcripts_info$ensembl_id <- gsub("\\(|\\)","",as.character(long_transcripts_info$ensembl_id))
...
counts_lm = 
  long_transcripts_info %>% 
  select(Gene, stripped_cell_line_name, primary_disease, spliceosome_mutated, log_TPM)
...
```{r}
keep <- c("Bone Cancer", "Brain Cancer", "Breast Cancer", "Colon/Colorectal Cancer", "Fibroblast", "Gastric Cancer", "Head and Neck Cancer", "Kidney Cancer", "Lung Cancer", "Neuroblastoma", "Ovarian Cancer", "Pancreatic Cancer", "Sarcoma", "Skin Cancer")
...
#mean_spliceosome_yes$ensembl_id <- gsub("\\(|\\)","",as.character(mean_spliceosome_yes$ensembl_id))

head(mean_spliceosome_yes)
```
...
lanc = 
  lm_spl %>% 
  left_join(mean_spliceosome_yes, by = c("ensembl_id") %>% 
...
```{r}
saveRDS(all_p.val, file = "res_lancaster/other_tcga_p_values_lancaster.rds")
```
