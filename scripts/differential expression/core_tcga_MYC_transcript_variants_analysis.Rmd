...
df_filter_pval %>% 
  ...
  ggtitle("log(TPM) expression of the isoforms of the gene RPS2")
```
Repeat for MYC 
```{r, fig.height=6, fig.width=8}

df_filter_pval %>% 
filter(Hugo_Symbol == "RPS2") %>%   drop_na(spliceosome_mutated) %>% 
  ggplot(aes(x = ensembl_id, y = log_TPM,fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
...
df_filter_pval %>% 
filter(Hugo_Symbol == "RPS2") %>% drop_na(spliceosome_mutated) %>% 
  mutate(is_canon = ifelse(ensembl_id == "ENST00000343262","canonical","alternative")) %>% 
...
df_filter_pval %>% 
  filter(Hugo_Symbol == "COX5A") %>% drop_na(spliceosome_mutated) %>% 
ggplot(aes(x = ensembl_id, y = log_TPM, fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
  ...
  ggtitle("log(TPM) expression of the isoforms of the gene COX5A")
```
...
df_filter_pval %>% 
filter(Hugo_Symbol == "COX5A") %>% drop_na(spliceosome_mutated) %>% 
  mutate(is_canon = ifelse(ensembl_id == "ENST00000322347","canonical","alternative")) %>% 
  ggplot(aes(x = is_canon, y = log_TPM,fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
...
df_filter_pval %>% 
  ...
  ggtitle("log(TPM) expression of the isoforms of the gene RNPS1")
```

```{r}
df_filter_pval %>% 
filter(Hugo_Symbol == "RNPS1") %>% drop_na(spliceosome_mutated) %>% 
  mutate(is_canon = ifelse(ensembl_id == "ENST00000565678","canonical","alternative")) %>% 
  ggplot(aes(x = is_canon, y = log_TPM,fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
...
```{r}
df_filter_pval %>% 
filter(Hugo_Symbol == "PABPC1") %>% drop_na(spliceosome_mutated) %>% 
  mutate(is_canon = ifelse(ensembl_id == "ENST00000318607","canonical","alternative")) %>% 
  ggplot(aes(x = is_canon, y = log_TPM,fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
...
df_filter_pval %>% 
  ...
  ggtitle("log(TPM) expression of the isoforms of the gene TXNL4A")
```

```{r}
df_filter_pval %>% 
filter(Hugo_Symbol == "TXNL4A") %>% drop_na(spliceosome_mutated) %>% 
  mutate(is_canon = ifelse(ensembl_id == "ENST00000269601","canonical","alternative")) %>% 
  ggplot(aes(x = is_canon, y = log_TPM,fill = spliceosome_mutated)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("NO" = "gray", "YES" = "red")) +
  stat_compare_means(label = "p.signif",method = "wilcox.test") +
